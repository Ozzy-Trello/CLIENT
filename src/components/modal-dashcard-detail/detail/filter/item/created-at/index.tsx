...
const CreatedAtItemFilter: FC<DashcardFilter> = ({ id, operator, value, label }) => {
  const {
    openEditFilter,
    currentFilter,
    handleChangeFilter,
  ...
  const options = useMemo(() => {
    return dashcardsFilter.find((filter) => filter.id === "created_at")?.options ?? [];
  }, []);
    ...
    return (
      <div className="flex items-center gap-3 justify-between">
        <div className="flex gap-3 items-center">
          <div className="font-semibold min-w-16">{label || "Created At"}</div>
        ...
        <Button
          icon={<DeleteOutlined />}
          onClick={() => handleDeleteFilter("created_at", id)}
        />
      </div>
    );
...
export default CreatedAtItemFilter;
