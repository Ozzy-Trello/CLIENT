...
const ModalPacking: React.FC<ModalPackingProps> = ({ open, onClose }) => {
  ...
  const handleBarcodeValidation = async (barcode: string) => {
    ...
    try {
      ...
      console.log("SO items:", currentSOForValidation.items?.map(item => ({
      ...
      if (foundItem && foundItem.product) {
        const productKey = foundItem.product.sku || foundItem.product.id?.toString() || "";
        ...
        setValidatedProducts(prev => new Set(Array.from(prev).concat(productKey)));
        message.success(`Produk ${foundItem.product.name} berhasil discan (${(scannedQuantities[productKey] || 0) + 1})`);
        setBarcodeInput("");
      } else {
        message.error(`Produk ${validationResult.data.product.name} tidak ditemukan dalam Sales Order ini`);
      }
    } catch (error) {
  ...
  const handleSOSelect = async (soNumber: string) => {
    try {
      ...
      if (rawSODetails) {
        ...
        const items = rawSODetails.purchaseOrderItems || rawSODetails.purchase_order_items || [];
        ...
        const itemsWithProducts = await Promise.all(
          items.map(async (item: any) => {
            ...
            return {
              ...
              whMaklonProductId: item.whMaklonProductId || item.wh_maklon_product_id,
         ...
         const transformedSODetails: OzzyPackingData = {
           ...
           whWarehouseId: rawSODetails.whWarehouseId || rawSODetails.wh_warehouse_id,
           ...
           whCustomerId: rawSODetails.whCustomerId || rawSODetails.wh_customer_id,
           ...
           purchaseOrder: {
             ...
             supplierName: rawSODetails.supplierName || rawSODetails.supplier_name,
             shippingAddress: rawSODetails.shippingAddress || rawSODetails.shipping_address,
             ...
             createdAt: rawSODetails.createdAt || rawSODetails.created_at,
             updatedAt: rawSODetails.updatedAt || rawSODetails.updated_at,
             accurateIdOzzy: rawSODetails.accurateIdOzzy || rawSODetails.accurate_id_ozzy,
             accurateIdMpi: rawSODetails.accurateIdMpi || rawSODetails.accurate_id_mpi,
           },
        ...
        <Form form={form} layout="vertical">
          ...
          <div>
            <Typography.Title level={5} className="mb-4">
              {selectedSalesOrders.length === 0 ? "Cari atau Scan Nomor SO" : "SO"}
            </Typography.Title>
              ...
              <Form.Item>
                <div className="flex gap-2">
                  <AutoComplete
                    ...
                    filterOption={false}
                    notFoundContent={
                      isLoading ? <Spin size="small" /> : "Sales Order tidak ditemukan"
                    }
                    className="flex-1"
                    size="large"
                  />
              ...
              <Form.Item>
                <div className="flex gap-2 items-center">
                  ...
                  <Button
                    ...
                    danger
                    size="large"
                    onClick={() => handleRemoveSalesOrder(selectedSalesOrders[0]?.soNumber)}
            ...
            <div className="mt-6">
              ...
              <Table
                dataSource={selectedSalesOrders.flatMap((so, soIndex) => 
                  ...
                  },
                  {
                    ...
                    render: (_, record) => (
                      <Button
                        ...
                        danger
                        size="small"
                        onClick={() => handleRemoveSalesOrder(record.soNumber)}
                      >
                        Hapus
                      </Button>
                    ),
            ...
            onClick={() => {
              if (currentSOForValidation) {
                const newSO: SelectedSalesOrder = {
                  ...
                  products: (currentSOForValidation.items ?? []).map((item: any) => ({
                ...
                setScannedQuantities({});
                setSOSearchValue("");
                message.success(`Sales Order ${newSO.soNumber} berhasil ditambahkan`);
              }
            }}
      ...
      >
        {currentSOForValidation && (
          <div className="space-y-4">
            ...
            <div>
              <Typography.Title level={5} className="mb-3">Scan Barcode Produk (Lusin/Pcs)</Typography.Title>
            ...
            <div>
              <Table
                columns={[
                  {
                    ...
                    align: "center",
                    render: (_, record) => {
                       const productKey = record.product?.sku || record.product?.id?.toString() || "";
                ...
                dataSource={currentSOForValidation.items?.map((item, index) => ({
                   ...item,
                   key: index,
                 }))}
                 pagination={false}
