...
export function formatCardDates(
  ...
  if (start && due) {
    ...
    if (start.isSame(due, "day")) {
      ...
      if (hasTime) {
        text = `${start.format(startFormat)} - ${due.format(
          endFormat
        )}, ${due.format("h:mm A")}`;
      } else {
 ...
 */
export function getDateStatusBadge(
  status: DateFormatResult["status"]
): string | null {
  switch (status) {
    case "overdue":
      return "Overdue";
...
export function CardDateDisplay({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {badge && (
        <span className={`px-1  rounded text-xs font-medium ${statusClasses}`}>
          {badge}
        </span>
      )}
    </div>
