...
import { getPOsByCardId, getPOScanProgress, PO, ScanProgressResponse } from "@api/po";
...
const ScanProgressModal: React.FC<ScanProgressModalProps> = ({
  ...
  const [scanProgress, setScanProgress] = useState<Record<string, ScanProgressResponse>>({});
  ...
  useEffect(() => {
    ...
    const hasChanged = 
      ...
      currentProgressKeys.some(key => 
        JSON.stringify(currentProgress[key]) !== JSON.stringify(scanProgress[key])
      );
  ...
  const handleScannerKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (value) {
        // For now, just clear the input - scanning functionality can be added later
        setScannerInput("");
  ...
  return (
    ...
    >
      ...
      <input
        ...
        style={{ position: "absolute", top: 0, left: 0, opacity: 0, width: "1px", height: "1px", border: "none", padding: 0, margin: 0 }}
        autoFocus
      />
        ...
        <div className="space-y-4">
          ...
          {scanProgressQueries.some(q => q.error) && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
              ...
              {scanProgressQueries.filter(q => q.error).map((query, index) => (
          ...
          {pos.length === 0 && (
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="text-yellow-800 text-sm">
                No PO items found for this card. Ensure the correct card ID was provided.
              </div>
            </div>
          )}

          {/* PO Scan Progress */}
          {pos.length > 0 && pos.slice().reverse().map((po: PO, idx: number) => {
            ...
            return (
              <div key={po.id} className="p-4 rounded-lg border bg-gray-50">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium">{scanned}/{total} ({percentage}%)</span>
                </div>
                <div className="mt-2">
