...
export function useCardDetails(cardId: string, listId: string) {
  ...
  const cardDetailsQuery = useQuery({
    ...
    initialData: () => {
      const cardsData = queryClient.getQueryData<ApiResponse<Card[]>>(["cards", listId]);
  ...
  const moveCardMutation = useMutation({
    mutationFn: ({ destinationListId, position }: { destinationListId: string, position?: number }) => {
    ...
    onMutate: async ({ destinationListId, position }) => {
      ...
      const sourceCards = queryClient.getQueryData<ApiResponse<Card[]>>(["cards", listId]);
      const destinationCards = queryClient.getQueryData<ApiResponse<Card[]>>(["cards", destinationListId]);
      ...
      queryClient.setQueryData(
        ...
        (old: ApiResponse<Card[]> | undefined) => {
          ...
          if (position !== undefined && position >= 0 && position <= newData.length) {
