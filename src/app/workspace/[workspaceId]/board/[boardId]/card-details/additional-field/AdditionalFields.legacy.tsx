...
import SummaryModalComponent from "./components-unused/SummaryModalComponent";
import SizesModalComponent from "./components-unused/SizesModalComponent";
import CustomSizePopoverComponent from "./components-unused/CustomSizePopoverComponent";
import JumlahPOComponent from "./components-unused/JumlahPOComponent";
import ScannerModalComponent from "./components-unused/ScannerModalComponent";
import TabContentComponent from "./components-unused/TabContentComponent";
import type {
  SizeBreakdown,
  SizesModalState,
  SummaryModalState,
...
const AdditionalFields: React.FC = () => {
  ...
  useEffect(() => {
    if (additionalFieldData && additionalFieldData.length > 0) {
      try {
        ...
        console.log("Loading additional field data:", savedData);
        ...
        } else if (savedData.storeData && savedData.storeData.data) {
          // Handle storeData structure from QR scans
          const items = savedData.storeData.data;
          console.log("Found storeData structure with items:", items);

          // Convert storeData items to poScannedItems format
          const convertedPoScannedItems: Record<number, ItemDetail[]> = {};

          items.forEach((item: any, index: number) => {
            // Extract PO number from item ID (e.g., "po-1753853709696-0" -> 1)
            const poMatch = item.id.match(/po-(\d+)-(\d+)/);
            const poNumber = poMatch ? parseInt(poMatch[2]) + 1 : 1; // Use the last number + 1 as PO number

            if (!convertedPoScannedItems[poNumber]) {
              convertedPoScannedItems[poNumber] = [];
            }

            // Convert item to ItemDetail format
            const convertedItem: ItemDetail = {
              id: item.id,
              butuhBahan: item.butuhBahan || false,
              additionalFields: item.detailProduk || {},
              usedAmount: 0,
              remainingAmount: 0,
              // Add size information from the item
              ...item,
            };

            convertedPoScannedItems[poNumber].push(convertedItem);
          });

          console.log("Converted to poScannedItems:", convertedPoScannedItems);
          setPoScannedItems(convertedPoScannedItems);

          // Set butuhBahan based on items
          const butuhBahanState: Record<number, boolean> = {};
          Object.keys(convertedPoScannedItems).forEach((poNumber) => {
            const poNum = parseInt(poNumber);
            butuhBahanState[poNum] = convertedPoScannedItems[poNum].some(
              (item) => item.butuhBahan
            );
          });
          setButuhBahan(butuhBahanState);

          // Set jumlahPO based on the number of POs
          setJumlahPO(Object.keys(convertedPoScannedItems).length);
        } else if (Array.isArray(savedData)) {
          // Old structure - array of items
          setScannedItems(savedData);
        }

        // Set the additional field ID for future updates
        setAdditionalFieldId(firstItem.id);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      // Only save if we have data and mutations are not pending
      if (
        (Object.keys(poScannedItems).length > 0 || scannedItems.length > 0) &&
        ...
        const currentData = {
          ...
          butuhBahan: butuhBahan,
          jumlahPO: jumlahPO,
          scannedItems: scannedItems, // Include legacy data for backward compatibility
        };
  ...
  }, [poScannedItems, scannedItems, butuhBahan, jumlahPO]); // Dependencies that trigger save
  ...
  // Function to save local sizes data to the database
  const handleSaveLocalSizes = (sizesData: Record<string, SizeBreakdown>) => {
    // Convert local sizes data to the format expected by the API
    const currentData = {
      poScannedItems: poScannedItems,
      butuhBahan: butuhBahan,
      jumlahPO: jumlahPO,
      localSizesData: sizesData, // Add the local sizes data
    };
  ...
  const updateSizeBreakdown = (
    ...
    setScannedItems((prevItems) => {
      ...
      return prevItems.map((item: ItemDetail, idx: number) => {
        if (idx !== itemIndex) return item;

        // Create new sizes object with defaults
        const currentSizes = item.additionalFields?.[sizesKey] || {
          XS: 0,
          S: 0,
          M: 0,
          L: 0,
        ...
        // Create new sizes object with updated values
        ...
        // Return new item with updated sizes
        return {
          ...item,
          additionalFields: {
    ...
    // Update poScannedItems
    setPoScannedItems((prevPoItems) => {
      if (!prevPoItems || Object.keys(prevPoItems).length === 0) {
        return prevPoItems;
      }

      const poId = Object.keys(prevPoItems)[0];
      const sizesKey = `sizes_${tabKey}_${fieldKey}`;

      return {
        ...prevPoItems,
        [poId]: (prevPoItems as any)[poId].map(
          (item: ItemDetail, idx: number) => {
            if (idx !== itemIndex) return item;
  ...
  useEffect(() => {
    if (itemDetail && currentScannedId) {
      ...
      setPoScannedItems((prevPoItems) => {
        ...
        // Save to database - save the multi-PO structure with current butuhBahan state
        // Use functional updates to get current state values
        setButuhBahan((currentButuhBahan) => {
          setJumlahPO((currentJumlahPO) => {
            const dataToSave = {
              poScannedItems: updatedPoScannedItems,
              butuhBahan: currentButuhBahan,
              jumlahPO: currentJumlahPO,
            };
            ...
            return currentJumlahPO; // Return unchanged
          });
          return currentButuhBahan; // Return unchanged
        });
      ...
      <div className="space-y-6">
        {Array.from({ length: jumlahPO }, (_, index) => (
          <TabContentComponent
            ...
            onSaveLocalSizes={handleSaveLocalSizes}
          />
        ))}
      </div>
