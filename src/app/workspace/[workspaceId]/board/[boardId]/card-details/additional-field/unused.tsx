...
const AdditionalFields: React.FC = () => {
  ...
  const updateSizeBreakdown = (
    ...
    setPoScannedItems((prevPoItems) => {
      ...
      const poId = parseInt(Object.keys(prevPoItems)[0]);
      const sizesKey = `sizes_${tabKey}_${fieldKey}`;

      return {
        ...prevPoItems,
        [poId]: prevPoItems[poId].map((item: ItemDetail, idx: number) => {
          if (idx !== itemIndex) return item;
  ...
  const handleCameraScan = (result: string) => {
    setCurrentScannedId(result);
    setShowScanner(false);
  };

  // Handle item detail when scanned
  useEffect(() => {
    if (itemDetail && currentScannedId) {
      ...
      // Add to the appropriate PO
      setPoScannedItems((prevPoItems) => {
        const updatedPoItems = { ...prevPoItems };
        if (!updatedPoItems[currentPoForScan]) {
          updatedPoItems[currentPoForScan] = [];
        }
        updatedPoItems[currentPoForScan] = [
          ...updatedPoItems[currentPoForScan],
          newItem,
      ...
      const currentData = {
        poScannedItems: {
          ...poScannedItems,
          [currentPoForScan]: [
            ...(poScannedItems[currentPoForScan] || []),
            newItem,
          ],
        },
        butuhBahan: butuhBahan,
  ...
  }, [itemDetail, currentScannedId]);

  // Handle remove tab
  const handleRemoveTab = (targetKey: string) => {
    // Check if targetKey contains PO identifier (e.g., "1_item_0")
    const poMatch = targetKey.match(/^(\d+)_item_(\d+)$/);

    if (poMatch) {
      const poIdentifier = parseInt(poMatch[1]);
      const itemIndex = parseInt(poMatch[2]);

      setPoScannedItems((prevPoItems) => {
        const updatedPoItems = { ...prevPoItems };
        if (updatedPoItems[poIdentifier]) {
          updatedPoItems[poIdentifier] = updatedPoItems[poIdentifier].filter(
