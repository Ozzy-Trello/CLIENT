  ...
  reducers: {
    ...
    addCard: (
      ...
      const newCard: Card = {
        ...
        isWatched: false,
        isArchived: false,
        isNew: true, // Flag to indicate this is a newly created card
        position:
          state.cards.filter((card) => card.id.startsWith(boardId)).length + 1,
        createdAt: now,
        updatedAt: now,
      };
    ...
    addCounterCard: (
      ...
      const { listId, boardId, title, count, filterCriteria, user } =
        action.payload;
      const now = formatDate();
