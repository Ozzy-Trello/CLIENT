...
const listSlice = createSlice({
  ...
  reducers: {
    ...
    addList: (
      ...
      const boardLists = state.lists.filter((list) =>
        list.id.startsWith(boardId)
      );
    ...
    updateList: (
      state,
      action: PayloadAction<Partial<AnyList> & { id: string }>
    ) => {
    ...
    reorderLists: (
      ...
      // Reorder the lists
      const reorderedLists = reorderItems(
        boardLists,
        sourceIndex,
        destinationIndex
      );
...
export const selectBoardLists = (state: RootState, boardId: string) => {
  return state.lists.lists
    .filter((list: AnyList) => {
      ...
      return (
        list.id.startsWith(`${boardId}-list-`) ||
