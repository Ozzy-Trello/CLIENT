...
export const useCardMembers = (cardId: string) => {
  ...
  const addMemberMutation = useMutation({
    mutationFn: ({ userIds }: { userIds: string[] }) =>
      apiAddMember(cardId, userIds as []),
    onMutate: async ({ userIds }) => {
      ...
      if (previousCardMembers?.data) {
        ...
        const optimisticUsers: User[] = userIds.map(
          (userId) =>
            ({
              id: userId,
        ...
        const newUsers = optimisticUsers.filter(
          (user) => !existingUserIds.includes(user.id)
        );

        const updatedMembers = [...previousCardMembers.data, ...newUsers];

        queryClient.setQueryData<ApiResponse<User[]>>(["cardMembers", cardId], {
          ...previousCardMembers,
          data: updatedMembers,
        });
      }
  ...
  const isMember = (userId: string): boolean => {
    ...
    // Ensure members is an array before calling .some()
  ...
  return {
    ...
    cardMembers: Array.isArray(cardMembersQuery.data?.data)
      ? cardMembersQuery.data.data
      : [],
    memberCount: Array.isArray(cardMembersQuery.data?.data)
      ? cardMembersQuery.data.data.length
      : 0,
