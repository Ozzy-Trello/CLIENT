...
import { addCardLabel, cards, getCardLabels, moveCard, removeLabelFromCard, updateCard } from "../api/card";
import { api } from "../api";
...
export const useCardLabels = (workspaceId: string, cardId: string) => {
  const queryClient = useQueryClient();

  // Main query for card labels
  const cardLabelsQuery = useQuery({
    queryKey: ["cardLabels", workspaceId, cardId],
    queryFn: () => getCardLabels(workspaceId, cardId),
    enabled: !!cardId && !!workspaceId,
    staleTime: 5000,
  });

  // Add card label mutation with optimistic update
  const addLabelMutation = useMutation({
    mutationFn: (labelId: string) => addCardLabel(workspaceId, labelId, cardId),
    onMutate: async (labelId: string) => {
      ...
      const previousCardLabels = queryClient.getQueryData<ApiResponse<any>>(
        ["cardLabels", workspaceId, cardId]
      );
      
      // Optimistically update to the new value
      if (previousCardLabels?.data) {
        // Check if label already exists to prevent duplicates
        const labelExists = previousCardLabels.data.some(
          (label: any) => label.id === labelId || label.labelId === labelId
        );
        ...
        if (!labelExists) {
          const newLabel = { 
            id: labelId, 
            labelId: labelId,
            // Add other label properties as needed based on your label structure
          };
          ...
          queryClient.setQueryData<ApiResponse<any>>(
            ...
            {
              ...previousCardLabels,
              data: [...previousCardLabels.data, newLabel]
            }
          );
        }
      }
  ...
  // Remove card label mutation with optimistic update
  const removeLabelMutation = useMutation({
    mutationFn: (labelId: string) => removeLabelFromCard(labelId, cardId),
    onMutate: async (labelId: string) => {
      // Cancel any outgoing refetches
      await queryClient.cancelQueries({ 
        queryKey: ["cardLabels", workspaceId, cardId] 
      ...
      // Optimistically update to the new value
      if (previousCardLabels?.data) {
        const updatedLabels = previousCardLabels.data.filter(
          (label: any) => label.id !== labelId && label.labelId !== labelId
        );
  ...
  // Helper function to check if a label is attached to the card
  const hasLabel = (labelId: string): boolean => {
    const labels = cardLabelsQuery.data?.data || [];
    return labels.some((label: any) => 
      label.id === labelId || label.labelId === labelId
    );
  };

  // Helper function to get a specific label
  const getLabel = (labelId: string): any => {
    const labels = cardLabelsQuery.data?.data || [];
  ...
  // Helper function to toggle a label (add if not present, remove if present)
  ...
  return {
    ...
    isLoading: cardLabelsQuery.isLoading,
    isError: cardLabelsQuery.isError,
    error: cardLabelsQuery.error,
    ...
    isUpdating: addLabelMutation.isPending || removeLabelMutation.isPending,
    addError: addLabelMutation.error,
    removeError: removeLabelMutation.error,
    ...
    addLabelAsync: addLabelMutation.mutateAsync,
    removeLabelAsync: removeLabelMutation.mutateAsync,

    // Raw mutations for advanced usage
    addLabelMutation: addLabelMutation.mutate,
    removeLabelMutation: removeLabelMutation.mutate,

    // For debugging
    refetch: cardLabelsQuery.refetch,
  };
};
